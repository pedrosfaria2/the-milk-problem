...
// TO DO - MESSAGING - Remove the @Ignore annotation
class AppRabbitTest {
    ...
    fun testSaferQuantity() {
        // TO DO - MESSAGING -
        // Test a "safer" purchase by using a different "safer" queue
        ...
        with(engine) {
            with(handleRequest(HttpMethod.Get, "/")) {
                val compactContent = response.content!!.replace("\\s".toRegex(), "")
                val milkCount = "<td>milk</td><td>([0-9]+)</td>".toRegex().find(compactContent)!!.groups[1]!!.value
                assertEquals(130, milkCount.toInt())
            }
        }
    }
